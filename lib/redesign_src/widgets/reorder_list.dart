...
/// A callback used by [CustomReorderableList] to report that a list item has moved
...
/// See also:
///
///  * [CustomReorderableList], a widget list that allows the user to reorder
///    its items.
///  * [CustomSliverReorderableList], a sliver list that allows the user to reorder
///    its items.
...
/// [CustomReorderableList] and [CustomSliverReorderableList].
///
...
/// [CustomReorderableListState.startItemDragReorder]. This is most easily achieved
/// by wrapping each child in a [CustomReorderableDragStartListener] or a
/// [CustomReorderableDelayedDragStartListener]. These will take care of recognizing
/// the start of a drag gesture and call the list state's
/// [CustomReorderableListState.startItemDragReorder] method.
///
/// This widget's [CustomReorderableListState] can be used to manually start an item
/// reorder, or cancel a current drag. To refer to the
/// [CustomReorderableListState] either provide a [GlobalKey] or use the static
/// [CustomReorderableList.of] method from an item's build method.
///
/// See also:
///
///  * [CustomSliverReorderableList], a sliver list that allows the user to reorder
...
class CustomReorderableList extends StatefulWidget {
  ...
  /// (usually a [CustomReorderableDragStartListener] or
  ...
  /// of the items.
  /// {@endtemplate}
  final CustomReorderCallback onReorder;
  ...
  /// {@endtemplate}
  final CustomReorderItemProxyDecorator? proxyDecorator;
  ...
  /// context.
  ///
  /// This method is typically used by [CustomReorderableList] item widgets that
  /// insert or remove items in response to user input.
  ///
  /// If no [CustomReorderableList] surrounds the given context, then this function
  ...
  ///    [CustomReorderableList] ancestor is found.
  static CustomReorderableListState of(BuildContext context) {
    final CustomReorderableListState? result = context.findAncestorStateOfType<CustomReorderableListState>();
    assert(() {
      if (result == null) {
  ...
  /// context.
  ///
  /// This method is typically used by [CustomReorderableList] item widgets that insert
  /// or remove items in response to user input.
  ///
  /// If no [CustomReorderableList] surrounds the context given, then this function will
  /// return null.
  ///
  /// This method can be expensive (it walks the element tree).
  ///
  /// See also:
  ///
  ///  * [of], a similar function that will throw if no [CustomReorderableList] ancestor
  ///    is found.
...
/// can refer to the [CustomReorderableList]'s state with a global key:
///
/// ```dart
...
class CustomReorderableListState extends State<CustomReorderableList> {
  ...
  /// [CustomReorderableDragStartListener] or [CustomReorderableDelayedDragStartListener]
  ...
  void startItemDragReorder({
    ...
    _sliverReorderableListKey.currentState!
        .startItemDragReorder(index: index, event: event, recognizer: recognizer);
  }
...
/// [CustomSliverReorderableListState.startItemDragReorder]. This is most easily
/// achieved by wrapping each child in a [CustomReorderableDragStartListener] or
/// a [CustomReorderableDelayedDragStartListener]. These will take care of
/// recognizing the start of a drag gesture and call the list state's start
/// item drag method.
///
/// This widget's [CustomSliverReorderableListState] can be used to manually start an item
/// reorder, or cancel a current drag that's already underway. To refer to the
/// [CustomSliverReorderableListState] either provide a [GlobalKey] or use the static
/// [CustomSliverReorderableList.of] method from an item's build method.
///
/// See also:
///
///  * [CustomReorderableList], a regular widget list that allows the user to reorder
///    its items.
///  * [ReorderableListView], a Material Design list that allows the user to
///    reorder its items.
class CustomSliverReorderableList extends StatefulWidget {
  ...
  /// context.
  ///
  /// This method is typically used by [CustomSliverReorderableList] item widgets to
  /// start or cancel an item drag operation.
  ///
  /// If no [CustomSliverReorderableList] surrounds the context given, this function
  /// will assert in debug mode and throw an exception in release mode.
  ///
  /// This method can be expensive (it walks the element tree).
  ///
  ...
  ///
  ///  * [maybeOf], a similar function that will return null if no
  ///    [CustomSliverReorderableList] ancestor is found.
  ...
  /// context.
  ///
  /// This method is typically used by [CustomSliverReorderableList] item widgets that
  /// insert or remove items in response to user input.
  ///
  /// If no [CustomSliverReorderableList] surrounds the context given, this function
  /// will return null.
  ...
  /// See also:
  ///
  ///  * [of], a similar function that will throw if no [CustomSliverReorderableList]
  ///    ancestor is found.
...
///
/// An app that needs to start a new item drag or cancel an existing one
/// can refer to the [CustomSliverReorderableList]'s state with a global key:
///
/// ```dart
...
/// ```
///
/// [CustomReorderableDragStartListener] and [CustomReorderableDelayedDragStartListener]
/// refer to their [CustomSliverReorderableList] with the static
/// [CustomSliverReorderableList.of] method.
class CustomSliverReorderableListState extends State<CustomSliverReorderableList>
  ...
  bool get _reverse =>
      _scrollable.axisDirection == AxisDirection.up || _scrollable.axisDirection == AxisDirection.left;
  ...
  void startItemDragReorder({
    ...
    setState(() {
      ...
      if (_items.containsKey(index)) {
        ...
        /// AFTERMVPTODO(darrenaustin): Can we handle this better, maybe scroll to the item?
    ...
    setState(() {
      if (_insertIndex == item.index) {
        ...
        if (_insertIndex! >= _items.length) {
          ...
          _finalDropPosition =
              _itemOffsetAt(_insertIndex!) + _extentOffset(_itemExtentAt(_insertIndex!), _scrollDirection);
        }
      } else {
        if (_insertIndex! == 0) {
  ...
  void _dragUpdateItems() {
    ...
    final double proxyItemStart =
        _offsetExtent(_dragInfo!.dragPosition - _dragInfo!.dragOffset, _scrollDirection);
...
/// See also:
///
///  * [CustomReorderableDelayedDragStartListener], a similar wrapper that will
...
/// See also:
///
///  * [CustomReorderableDragStartListener], a similar wrapper that will
...
class _DragInfo extends Drag {
  _DragInfo({
    ...
    /// AFTERMVPTODO (polina-c): stop duplicating code across disposables
